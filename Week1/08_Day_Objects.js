...
const personAccount = {
  ...
  accountBalance() {
    return `You have ${this.totalIncome() - this.totalExpense()} left in your account.`
  }
}
...
function rateProduct(productId, rating) {
  if (!currentUser || !currentUser.isLoggedIn) return "Sign in to rate this product"
  products.forEach(({ _id, ratings }) => {
    if (productId === _id) {
      ratings = [...ratings, { userId: currentUser._id, rate: rating}]
    }
  })
  return "User rating has been submitted"
}
